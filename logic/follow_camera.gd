...
func _physics_process(delta):
	...
	var linear_velocity = get_carbody().linear_velocity
	var fwd_mps = get_carbody().transform.basis.xform_inv(linear_velocity).z
	var angular_velocity = get_carbody().angular_velocity
	...
	if timer_0_5s < 0:
		if get_carbody().vehicle_state == "alive":  # else if the car has started exploding leave it at 3rd person for now TODO fix later? 
			var iray
