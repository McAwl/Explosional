...
func _ready():
	...
	if build == null or version == null:
		$VersionText/VersionContainer/VersionText.text = "Explosional v1.8.13-beta 2024 McAwl"
	else:
		if len(build) < 2 or len(version) < 2:
			$VersionText/VersionContainer/VersionText.text = "Explosional v1.8.13-beta 2024 McAwl "  # + str(len(version)) + " " + str(len(build))
		else:
